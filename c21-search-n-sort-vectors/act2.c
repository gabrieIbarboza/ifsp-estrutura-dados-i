...
// Funcao para inserir um novo valor no vetor ordenado
void inserirValor(int *vetor, int *n, int valor) {
    int i;
    // Encontra a posicao correta para o novo valor
    for (i = *n - 1; i >= 0 && vetor[i] > valor; i--) {
        vetor[i + 1] = vetor[i]; // Desloca os elementos maiores para a direita
    }
    vetor[i + 1] = valor; // Insere o novo valor na posicao correta
...
int main() {
    ...
    // Insercao de novo valor no vetor ordenado
    int elemNovo = 135;
    inserirValor(vetorOrdenado, &tamanhoOrdenado, elemNovo);
    printf("Elemento a ser inserido no vetor ordenado: %d\n", elemNovo);

    printf("Vetor Ordenado Atualizado: ");
    imprimirVetor(vetorOrdenado, tamanhoOrdenado);
